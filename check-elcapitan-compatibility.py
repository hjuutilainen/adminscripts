...
# ================================================================================
# check-elcapitan-compatibility.py
#
# This script checks if the current system is compatible with OS X 10.11 El Capitan.
# These checks are based on the installCheckScript and volCheckScript in
# /Applications/Install OS X El Capitan.app/Contents/SharedSupport/OSInstall.mpkg/Distribution
#
# The checks used in this script are:
# - Machine has a specific supported board-id or is a virtual machine
# - 64 bit capable CPU
# - At least 2 GB of memory
# - Current system version is less than 10.11
# - Current system version is at least 10.6.8 or newer
#
# Exit codes:
# 0 = El Capitan is supported
# 1 = El Capitan is not supported
#
#
...
def is_system_version_supported():
    ...
    if StrictVersion(product_version) >= StrictVersion('10.11'):
        logger("System",
               "%s %s" % (product_name, product_version),
               "Failed")
        return False
    elif StrictVersion(product_version) >= StrictVersion('10.6.8'):
        logger("System",
               "%s %s" % (product_name, product_version),
               "OK")
        return True
...
def main(argv=None):
    elcapitan_supported_dict = {}
    elcapitan_needs_fw_update_dict = {}
    ...
    if board_id_passed and memory_passed and cpu_passed and system_version_passed:
        elcapitan_supported = 0
        elcapitan_supported_dict = {'elcapitan_supported': True}
    else:
        elcapitan_supported = 1
        elcapitan_supported_dict = {'elcapitan_supported': False}

    # Update "ConditionalItems.plist" if munki is installed
    if munki_installed() and update_munki_conditional_items:
        append_conditional_items(elcapitan_supported_dict)

    # Exit codes:
    # 0 = El Capitan is supported
    # 1 = El Capitan is not supported
    return elcapitan_supported
